...
## add the "DE" column separately
miRNA_results$DE <- "not DE"
miRNA_results$DE[miRNA_results$log2FoldChange > 1 & miRNA_results$padj < 0.05] <- "up"
miRNA_results$DE[miRNA_results$log2FoldChange < -1 & miRNA_results$padj < 0.05] <- "down"
...
## Heatmaps for the significant genes (Fig 3a)
```{r}
...
## read in the lncRNAs to make a heatmap for them (Fig 2a)
```{r}
sig_lncRNAs <- read.delim(paste0(lncRNA_input, "norm_counts_all_significant_lncRNAs.txt"),
                          sep = "\t")
...
pdf(file = paste0(output_dir, "Heatmap_sig_lncRNAs_log2_scaled_pearson_new.pdf"),
    width = 7, height = 7)
## without legend
...
DE_lncRNA_targets <- read.table(paste0(lncRNA_input, "all_correlating_DE_target_genes_lncRNAs_stats.txt"),
                                header = TRUE, sep = " ")
## try to remove the ones with adj. p-value > 0.05
## only WDFY3-AS2 has correlating targets
DE_lncRNA_targets <- DE_lncRNA_targets[-which(DE_lncRNA_targets$lncRNA == "WDFY3-AS2"), ]
length(unique(DE_lncRNA_targets$target))
table(table(DE_lncRNA_targets$target))
## miRNAs
...
write.table(sig_lncRNA_targets_Reactome, paste0(lncRNA_input, "lncRNA_pathways_new.txt"),
            sep = "\t", row.names = FALSE, quote = FALSE)
test <- read.delim(paste0(lncRNA_input, "lncRNA_pathways_new.txt"))
test_2 <- unlist(strsplit(test$Genes, split = ";"))
...
heatmap_col <- c("#FFFFFF", "#d5d3d3")
names(heatmap_col) <- c("no", "yes")
pdf(file = paste0(output_dir, "lncRNA_target_enrichment_Reactome_filtered_new.pdf"),
    height = 5, width = 15)
...
write.table(sig_miRNA_targets_Reactome, paste0(lncRNA_input, "miRNA_pathways_new.txt"),
            sep = "\t", row.names = FALSE, quote = FALSE)
...
heatmap_col <- c("#FFFFFF", "#d5d3d3")
names(heatmap_col) <- c("no", "yes")
pdf(file = paste0(output_dir, "miRNA_target_enrichment_Reactome_filtered_new.pdf"),
    height = 5, width = 15)
print(Heatmap(heatmap_df_filtered, top_annotation = top_anno,
        bottom_annotation = bottom_anno_3,
        right_annotation = right_annot, col = heatmap_col,
...
miRNA_gene_names <- c("MIRLET7B", "MIR383", "MIR708", "MIR210", "MIR301B",
                      "MIR15B", "MIR187", "MIR199A", "MIR199B", "MIR4421",
                      "MIR4717", "MIR4741", "MIR4746", "MIR548AB", "MIR7702",
                      "MIR505", "MIR199B", "MIR1298", "MIR4662A")
...
## put to xlsx
write.xlsx(GLASS_CNAs, file = paste0(GLASS_input, "GLASS_CNAs_miRNAs.xlsx"))
...
norm_counts_diffuse_astros <- read.delim(paste0(miRNA_input, "TCGA_primary_norm_counts_GBMs_oligos_included.txt"))
...
lncRNAs_TCGA <- lncRNAs_TCGA[, -c(1, 579)]
lncRNAs_TCGA <- log2(lncRNAs_TCGA + 1)
## also need the files whether they are grade 2 or grade 3
TCGA_classification <- read.delim(paste0(miRNA_input, "TCGA_classification_for_violins_all_primary_tumors.txt"),
                                  sep = "\t")
...
TCGA_miRNA_norm_counts <- read.delim(paste0(miRNA_input, "TCGA_miRNA_primary_norm_counts_GBMs_oligos_included.txt"))

## miRNAs are the rownames
## filter for the wanted genes
miRNAs <- c("hsa-let-7b", "hsa-mir-15b")
...
TCGA_miRNA_classification <- read.delim(paste0(miRNA_input, "TCGA_classification_for_violins_all_primary_tumors_miRNAs.txt"),
                                        sep = "\t")
...
rownames(TCGA_CNAs) <- TCGA_CNAs$SAMPLE_ID
## filter for the respective genes
miRNA_gene_names <- c("LET.7B.3P", "MIR.15B.5P")
miRNA_CNAs <- TCGA_CNAs[, which(colnames(TCGA_CNAs) %in% miRNA_gene_names)]
colnames(miRNA_CNAs) <- c("hsa-let-7b", "hsa-mir-15b")
...
## expression barplots for the lncRNAs (Fig 2c)
...
genes <- c("hsa-let-7b-3p", "hsa-miR-15b-5p")
ylims <- list(NULL)
...
red_colors <- c("#f0e5e5", "#e57373", "#b71c1c")
